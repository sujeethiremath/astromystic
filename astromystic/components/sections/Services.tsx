...
export default function Services({ currentStyles, theme, onBookNow }: ServicesProps) {
  ...
  return (
    <section id="services" className={`py-24 px-4 ${theme === 'sun' ? 'bg-amber-100/50' : 'bg-slate-900/50'}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {SERVICES.map((service) => (
            <div key={service.id} className={`p-8 rounded-2xl border ${currentStyles.cardBg} ${currentStyles.cardBorder}`}>
