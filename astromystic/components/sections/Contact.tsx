...
export default function Contact({ theme }: ContactProps) {
  ...
  return (
    <section id="contact" className={`py-12 md:py-24 px-4 relative overflow-hidden ${current.bg} ${current.text}`}>
      
      {/* Decorative Background Glows (Subtle) */}
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
         <div className={`absolute -top-20 -right-20 w-96 h-96 rounded-full filter blur-3xl opacity-10 ${theme === 'sun' ? 'bg-amber-300' : 'bg-indigo-600'}`} />
         <div className={`absolute -bottom-20 -left-20 w-72 h-72 rounded-full filter blur-3xl opacity-10 ${theme === 'sun' ? 'bg-yellow-300' : 'bg-purple-600'}`} />
      </div>
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="text-center space-y-4 mb-12">
          <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-current border-opacity-20 text-xs uppercase tracking-widest opacity-80`}>
            <Sparkles className="w-3 h-3" />
            <span>Get in Touch</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-serif font-bold">Chart Your Course</h2>
        ...
        {/* Form Card */}
        <div className={`max-w-xl mx-auto p-8 rounded-3xl border shadow-xl ${current.cardBg} ${current.border}`}>
          <form className="space-y-6" onSubmit={handleSubmit}>
            
            {/* Name Input */}
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest opacity-60 font-bold ml-1">Name</label>
              <div className="relative group">
                <User className={`absolute left-4 top-3.5 w-5 h-5 transition-colors ${current.icon} opacity-50 group-focus-within:opacity-100`} />
                <input 
                  type="text" 
                  name="name" 
                  value={formData.name} 
            ...
            {/* Email Input */}
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest opacity-60 font-bold ml-1">Email</label>
              <div className="relative group">
                <Mail className={`absolute left-4 top-3.5 w-5 h-5 transition-colors ${current.icon} opacity-50 group-focus-within:opacity-100`} />
                <input 
                  type="email" 
                  name="email" 
            ...
            {/* Message Input */}
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest opacity-60 font-bold ml-1">Message</label>
              <div className="relative group">
                <MessageSquare className={`absolute left-4 top-3.5 w-5 h-5 transition-colors ${current.icon} opacity-50 group-focus-within:opacity-100`} />
                <textarea 
                  rows={4}
                  name="message" 
            ...
            >
              {status === 'loading' ? (
                <span className="animate-pulse">Sending across the cosmos...</span>
              ) : (
