...
export default function AuthModal({ isOpen, onClose, theme }: AuthModalProps) {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        setMode('login');
        setSuccessMsg(
          `Verification sent to ${email}. Please verify before logging in.`
        );
      }
    } catch (err: any) {
      let msg = err.message;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div
        className={`relative w-full max-w-md p-8 rounded-2xl shadow-2xl ${current.bg} ${current.text} border ${current.border} max-h-[90vh] overflow-y-auto`}
      >
        <button
          onClick={onClose}
        ...
        <p className="text-center opacity-70 mb-6 text-sm">
          {mode === 'login'
            ...
            : mode === 'signup'
              ? 'Create an account to book your journey'
              : 'Recover your account access'}
        </p>

        {successMsg && (
          <div className="mb-4 p-3 rounded bg-green-500/20 text-green-600 text-sm text-center">
            {successMsg}
          </div>
        )}
        {error && (
          <div className="mb-4 p-3 rounded bg-red-500/20 text-red-600 text-sm text-center">
            {error}
          </div>
        )}

        <form
          onSubmit={mode === 'forgot' ? handleResetPassword : handleEmailAuth}
          className="space-y-4"
        >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-3 rounded-lg outline-none focus:ring-2 ring-opacity-50 ${current.inputBg} ${current.border}`}
                  required
                />
              </div>
              <div className="relative">
          ...
          {mode === 'login' && (
            <div className="text-right">
              <button
                ...
                className={`text-xs font-bold ${current.link}`}
              >
                Forgot Password?
              </button>
            </div>
          ...
          <button
            disabled={loading}
            className={`w-full py-3 rounded-lg font-bold tracking-wide ${current.button} ${loading ? 'opacity-50' : ''}`}
          >
            {loading
              ? 'Processing...'
              : mode === 'login'
        ...
        {mode !== 'forgot' && (
          <>
            ...
            <button
              ...
              className="w-full py-3 rounded-lg font-bold border border-current border-opacity-20 hover:bg-current hover:bg-opacity-5 flex items-center justify-center gap-3"
            >
              {/* Google Logo */}
            ...
            <div className="mt-6 text-center text-sm">
              <span className="opacity-70">
                {mode === 'login' ? 'No account?' : 'Has account?'}
              </span>
              <button
                onClick={() =>
                  switchMode(mode === 'login' ? 'signup' : 'login')
                }
                className={`ml-2 font-bold ${current.accent}`}
              >
        ...
        {mode === 'forgot' && (
          <div className="mt-6 text-center">
            <button
              ...
              className={`flex items-center justify-center gap-2 mx-auto ${current.link}`}
            >
